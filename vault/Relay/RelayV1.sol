...
contract Relayer is ILayerZeroRelayer, ReentrancyGuard, OwnableUpgradeable, Proxied {
    ...
    // [_chainId][_outboundProofType] => DstPriceData. change often
    mapping(uint16 => mapping(uint16 => DstPrice)) public dstPriceLookup;
    // [_chainId] => DstConfig. change much less often
    mapping(uint16 => DstConfig) public dstConfigLookup;
    ...
    function setDstPrice(uint16 _chainId, uint16 _outboundProofType, uint128 _dstPriceRatio, uint128 _dstGasPriceInWei) external onlyApproved {
        dstPriceLookup[_chainId][_outboundProofType] = DstPrice(_dstPriceRatio, _dstGasPriceInWei);
    }

    function setDstConfig(uint16 _chainId, uint128 _dstNativeAmtCap, uint64 _baseGas, uint64 _gasPerByte) external onlyApproved {
        dstConfigLookup[_chainId] = DstConfig(_dstNativeAmtCap, _baseGas, _gasPerByte);
    }

    // other relayer fees are withdrawn through the RelayerFee interface
    // uint8 public constant WITHDRAW_TYPE_RELAYER_QUOTED_FEES = 3;
    function withdrawQuotedFromULN(address payable _to, uint _amount) external onlyApproved {
        uln.withdrawNative(3, address(0x0), _to, _amount);
    }
    ...
    function _getPrices(uint16 _dstChainId, uint16 _outboundProofType, address, bytes memory _adapterParameters) internal view returns (uint basePrice, uint pricePerByte) {
        // decoding the _adapterParameters
        ...
        DstPrice storage dstPrice = dstPriceLookup[_dstChainId][_outboundProofType];
        DstConfig storage dstConfig = dstConfigLookup[_dstChainId];
